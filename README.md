...
- ~~블록 설치/파괴 — 좌클릭=파괴(AIR), 우클릭=설치(STONE)~~ → **④에서 은퇴**(지형 불변)
- 지형 다양화 — 물(저지대 호수), 바이옴(GRASS/DIRT)
- 핫바 UI — 스크린 좌표 전용 파이프라인(depth off, alpha blend), 9칸 슬롯 + 선택 강조, 각 슬롯에 블록 색상 아이콘
- 블록 타입 선택 — 숫자키 1~9 / 스크롤 휠로 슬롯 선택 (블록 설치는 ④에서 은퇴, 슬롯 선택은 도구/아이템에 유지)
...
- 제네릭 오브젝트 시스템 — 나무 전용 → 타입 인덱싱 메시 레지스트리(`m_objectMeshes`) + 청크별 타입 그룹. 데이터 주도 `ObjectDef`(castShadow/collidable/placeable/harvestTool/dropItem)로 타입별 속성 정의. ROCK(로우폴리 돌덩이) 타입 + `placeRocks` 절차 배치 추가
- 자원 채집 — 도끼로 나무, 곡괭이로 돌 좌클릭 시 오브젝트 제거 + `ObjectDef.dropItem` 드롭 → 줍기. 도구 안 맞으면 채집 안 됨(지형 보호). 채집물 미저장(결정론 재생성, 세션 내 유지)
- 지형 불변(복셀 편집 은퇴) — 좌클릭 타일 파괴·우클릭 블록 설치 제거. 좌클릭=채집/수확, 우클릭=농사 도구만. 지형은 authored·고정, 건축은 ⑥ 오브젝트 레이어 예정
- 인벤토리 제작 — 스타듀식 클릭형 레시피. `Recipe` 테이블(데이터 주도) + `countItem`/`removeItem`/`craft`. 인벤 창에 제작 패널(결과+입력 swatch, 재료 부족 시 어둡게), 클릭 시 재료 소모 후 제작. 작업대(WOOD×4)·울타리(WOOD×2). 작업대 전용 고급 레시피는 ⑤b 예정
...
- Settings 1차 App-state — MainMenu에서 `S`로 Settings 화면 진입, `ESC`/`BACK`으로 MainMenu 복귀. 설정 row 클릭으로 VSync ON/OFF(실제 present mode 적용), AA OFF/FXAA/SMAA(데이터/UI) 변경
...
- **Tier 1**: ✅ DevUI(ImGui) + GPU 프로파일링 · ✅ `FrameRenderData` 스냅샷 · ✅ `GpuBuffer` RAII · App-state(✅ MainMenu 1차 · ✅ Settings 클릭 UI(+VSync 적용/AA 데이터) · ✅ Loading 1차 · ✅ Pause 1차 / 추가 옵션 예정)
