...
순서: ✅① 인벤토리 모델 + 숫자 렌더러 + 작물 경제(씨앗 소모/수확 획득) → ✅② 제네릭 오브젝트 시스템 → ✅③ 자원 오브젝트+채집 → ✅④ 지형 불변화(복셀 설치/파괴 제거) → ⑤ 작업대/제작 → ⑥ 오브젝트 설치/철거. **(현재 ⑤ 직전)**
> ① 완료: 스택+개수 인벤토리, 숫자 렌더러, 낫 수확 + 드롭/줍기 레이어(`DroppedItem`). ② 완료: `m_objectMeshes` 메시 레지스트리 + 청크별 타입 그룹 + `ObjectDef` 테이블(TREE/ROCK). ③ 완료: `tryHarvestObject`(도끼→나무/곡괭이→돌 → 드롭 → 줍기). ④ 완료: 좌클릭 파괴·우클릭 복셀 설치 제거(지형 불변).
