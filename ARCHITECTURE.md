...
- **식생**: 절차 grass alpha texture + X자 card mesh + 청크별 grass instance buffer + 좌표 기반 density field. 시각 dressing layer이며 shadow caster는 아님
...
- 다음 비주얼 후보: organic dressing layer(잔돌/흙 패치/길 가장자리) · visual variant(tint/texture/card) · wind field · AA(SMAA 주력 + FXAA fallback) · LUT(선택)
...
- 현재 상태: 절차 RGBA grass texture, alpha-test grass pipeline, X자 card mesh, 청크별 instance buffer, 좌표 기반 density field까지 1차 연결 완료. 다음 개선은 ground dressing layer, tint/texture/card variant, wind 중심.
...
- **grass dressing**: alpha card + density field 1차는 완료. 다음 핵심은 잔돌/흙 패치 같은 ground dressing layer, tint/texture/card variant, wind.
