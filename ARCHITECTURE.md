...
- App-state 머신(Boot/MainMenu/Loading/Gameplay/Pause) + 입력 컨텍스트 + 설정(해상도/vsync/볼륨/AA) + world load/unload. **Pause 1차 완료**: `ESC`로 Gameplay/Paused 토글, pause 중 게임 업데이트·카메라 회전 차단. 메인메뉴/로딩/설정은 예정
