...
- 다음 비주얼 후보: ground dressing placeholder 축소/텍스처화 · visual variant(tint/texture/card) · wind field · AA(SMAA 주력 + FXAA fallback) · LUT(선택)
...
- 현재 상태: 절차 RGBA grass texture, alpha-test grass pipeline, X자 card mesh, 청크별 instance buffer, 좌표 기반 density field까지 1차 연결 완료. ground dressing은 시각 placeholder가 과해 최종 방향으로 쓰기 어렵지만, 저장하지 않는 좌표 기반 placement layer로는 유효하다. 다음 개선은 placeholder를 낮은 대비의 texture/card detail로 바꾸고, tint/texture/card variant와 wind를 추가하는 방향.
...
- **grass/ground dressing**: alpha card + density field 1차는 완료. ground dressing 1차는 구조 검증에는 성공했지만 갈색 patch/pebble placeholder가 너무 크고 대비가 강해 화면이 지저분하다. 다음 핵심은 geometry 밀도 축소, 텍스처/알파 기반 디테일 전환, tint/texture/card variant, wind.
