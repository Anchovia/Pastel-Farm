...
- App-state 머신(Boot/MainMenu/Loading/Gameplay/Pause) + 입력 컨텍스트 + 설정(해상도/vsync/볼륨/AA) + world load/unload. **MainMenu 1차 + Pause 1차 완료**: 시작 시 MainMenu 표시, `Enter`로 Gameplay 진입, `ESC`로 Gameplay/Paused 토글. 메뉴/pause 중 게임 업데이트·카메라 회전·월드 입력 차단, dim overlay + pause 아이콘 + `PAUSED` 문구 표시. 입력 정책 helper와 로컬 `AppFlow`, Tiny UI Text 기반으로 DevUI 캡처/app mode 차단/edge-detect/기초 문구 표시 접합면을 정리. 로딩/설정은 예정
